...
const quests = frameworksData.map((f) => ({
    id: f.id,
    name: f.title,
    category: "Framework",
    concept: f.quote,
    example: f.takeaway,
    origin: "The Shrine Codex",
    xp: 50,
...
let state = { streak: 0, lastCompletionDate: null, spellbook: [], parkingLot: "" };
...
function loadState() {
    ...
    if (saved) {
        try {
            ...
            state = { ...state, ...parsed, spellbook: parsed.spellbook || [], parkingLot: parsed.parkingLot || "" };
        } catch (error) { console.error(error); }
    }
}

function saveState() {
    localStorage.setItem(STORAGE_KEY, JSON.stringify({ ...state, xpBalance, totalXP: xpBalance }));
}
...
function animateXp(from, to, duration = 700) {
    ...
    const step = (now) => {
        ...
        renderXp(Math.round(from + delta * eased));
...
function renderWisdomJourney(quest) {
    ...
    [
        { title: "Interpretation", text: "Summarize this framework in one sentence for your team." },
        { title: "Application", text: "Apply this framework to a current decision." },
        { title: "Reflection", text: "What blind spot does this framework reveal?" }
    ].forEach((item) => {
...
function handleCorrectAnswer(button, feedbackEl) {
    ...
    if (state.lastCompletionDate === null) state.streak = 1;
    else {
        const diffDays = (new Date(today) - new Date(state.lastCompletionDate)) / 86400000;
    ...
    requestAnimationFrame(() => pdfButton.classList.remove("opacity-0"));
}
...
function renderTrial() {
    ...
    activeTrial.options.forEach((text, index) => {
        const button = document.createElement("button");
        button.type = "button";
        button.className = "trial-option-btn";
        button.innerHTML = `<span class=\"font-label text-xs uppercase tracking-wider mr-3\">${String.fromCharCode(65 + index)}</span><span>${text}</span>`;
        button.addEventListener("click", () => {
            if (questState === "completed") return;
            selectedAnswer = index;
            if (index === activeTrial.correctIndex) handleCorrectAnswer(button, feedbackEl);
            else handleWrongAnswer(button, feedbackEl);
        });
...
function swapToNextQuest() {
    ...
    setTimeout(() => {
        currentQuest = getRandomQuest(currentQuest?.id ?? null);
        questState = "learning";
        selectedAnswer = null;
        renderQuest();
...
function toggleSpellbook() {
    ...
    } else state.spellbook.push(currentQuest.id);
    saveState();
    updateSpellbookButton();
}
...
function renderSpellbook() {
    ...
    if (bookmarked.length === 0) { list.innerHTML = "<li>Your spellbook is empty. Add frameworks you love!</li>"; return; }
...
function setupEvents() {
    ...
    document.getElementById("showSpellbookBtn").addEventListener("click", () => { renderSpellbook(); openModal("spellbookModal"); });
    document.getElementById("showParkingLotBtn").addEventListener("click", () => { document.getElementById("parkingLotText").value = state.parkingLot; openModal("parkingLotModal"); });
    document.getElementById("showQuestLogBtn").addEventListener("click", () => { renderQuestLog(); openModal("questLogModal"); });
